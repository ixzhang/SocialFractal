#' Calculate Clumping Rate in Social Networks in Null Model2
#'
#' This function calculates the clumping rate in a social network based on
#' spatial proximity over time, tests for the effect of spatial preferences
#' (which perch to use) on who individuals interact with by permuting the
#' individual identities across perches.
...
#' # threshold = 75)
#' # print(cr_matrix)
clumping_rate_null2 <- function(data, individuals, threshold) {
  ...
  for (time_idx in seq_along(time_points)) {
    ...
    individual_map$order <- sample(individual_map$order)
